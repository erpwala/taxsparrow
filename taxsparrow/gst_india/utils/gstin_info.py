...
from taxsparrow.gst_india.api_classes.public import PublicAPI
from taxsparrow.gst_india.utils import titlecase, validate_gstin
...
def get_gstin_info(gstin, *, throw_error=True):
    ...
    if not response:
        try:
            ...
            frappe.enqueue(
                "taxsparrow.gst_india.doctype.gstin.gstin.create_or_update_gstin_status",
                queue="long",
                response=response,
            )
