    ...
    frappe.ui.form.on(doctype, {
        after_save(frm) {
            ...
            frappe.confirm(message, function () {
                frappe.call({
                    method: "taxsparrow.gst_india.overrides.party.update_docs_with_previous_gstin",
                    args: {
                        gstin: gstin || "",
                        gst_category,
                        docs_with_previous_gstin,
...
function set_gstin_options_and_status(doctype) {
    frappe.ui.form.on(doctype, {
        refresh(frm) {
            ...
            taxsparrow.set_gstin_status(frm.get_field("gstin"));
        },
        gstin(frm) {
            taxsparrow.set_gstin_status(frm.get_field("gstin"));
        },
...
async function set_gstin_options(frm) {
    ...
    field.set_data(await taxsparrow.get_gstin_options(frm.doc.name, frm.doctype));
}
...
function set_gst_category(doctype) {
    frappe.ui.form.on(doctype, {
        gstin(frm) {
            frm.set_value(
                "gst_category",
                taxsparrow.guess_gst_category(frm.doc.gstin, frm.doc.country)
            );
        },
    });
}
