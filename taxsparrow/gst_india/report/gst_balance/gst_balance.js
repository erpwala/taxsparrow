...
async function set_gstin_options(report) {
    const options = await taxsparrow.get_gstin_options(
...
function add_custom_button_to_update_gstin(report) {
    frappe
        .call({
            method: "taxsparrow.gst_india.report.gst_balance.gst_balance.get_pending_voucher_types",
            args: {
...
async function update_gstin_message(report, voucher_types) {
    ...
    frappe.msgprint({
        ...
        primary_action: {
            ...
            server_action:
                "taxsparrow.gst_india.report.gst_balance.gst_balance.update_company_gstin",
