// Copyright (c) 2022, Sparrownova and contributors
...
frappe.ui.form.on("Purchase Reconciliation Tool", {
    async setup(frm) {
        ...
        new taxsparrow.quick_info_popover(frm, tooltip_info);
...
class PurchaseReconciliationTool {
    ...
    setup_filter_button() {
        this.filter_group = new taxsparrow.FilterGroup({
    ...
    apply_filters(force, supplier_filter) {
        ...
        this.filtered_data = this.data.filter(row => {
            return filters.every(filter =>
                taxsparrow.FILTER_OPERATORS[filter[2]](
                    filter[3] || "",
                    row[filter[1]] || ""
                )
            );
    ...
    render_data_tables() {
        this._tabs.forEach(tab => {
            this.tabs[`${tab}_tab`] = new taxsparrow.DataTableManager({
    ...
    export_data(selected_row) {
        ...
        const url =
            "taxsparrow.gst_india.doctype.purchase_reconciliation_tool.purchase_reconciliation_tool.download_excel_report";
...
class DetailViewDialog {
    ...
    render_cards() {
        ...
        new taxsparrow.NumberCardManager({
...
class ImportDialog {
    ...
    async download_gstr(only_missing = true, otp = null) {
        ...
        if (message && ["otp_requested", "invalid_otp"].includes(message.error_type)) {
            const otp = await taxsparrow.get_gstin_otp(message.error_type);
...
class EmailDialog {
    ...
    get_attachment() {
        ...
        frappe.call({
            method: "taxsparrow.gst_india.doctype.purchase_reconciliation_tool.purchase_reconciliation_tool.generate_excel_attachment",
            args: {
    ...
    async get_recipients() {
        const { message } = await frappe.call({
            method: "taxsparrow.gst_india.utils.get_party_contact_details",
            args: {
...
async function create_new_purchase_invoice(row, company, company_gstin) {
    ...
    const { message: supplier } = await frappe.call({
        method: "taxsparrow.gst_india.utils.get_party_for_gstin",
        args: {
...
async function set_gstin_options(frm) {
    const { query, params } = taxsparrow.get_gstin_query(frm.doc.company);
    const { message } = await frappe.call({
        method: query,
        args: params,
    });
