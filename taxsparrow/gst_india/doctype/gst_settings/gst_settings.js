...
frappe.ui.form.on("GST Settings", {
    setup(frm) {
        ...
        frm.set_query("gstin", "credentials", (_, cdt, cdn) => {
            ...
            return taxsparrow.get_gstin_query(row.company);
        });
    },
...
function show_ic_api_promo(frm) {
    ...
    const alert = $(`
        ...
        >
            <div>
                ...
                <a href="/app/tax-sparrow" class="alert-link">
                    Get started with the Tax Sparrow API!
                </a>
            </div>
            <button
                type="button"
    ...
    alert.on("closed.bs.alert", () => {
        frappe.xcall(
            "taxsparrow.gst_india.doctype.gst_settings.gst_settings.disable_api_promo"
        );
    });
}
    ...
    frm.add_custom_button(__("Update GST Category"), () => {
        frappe.msgprint({
            ...
            primary_action: {
                label: __("Update"),
                server_action:
                    "taxsparrow.gst_india.doctype.gst_settings.gst_settings.enqueue_update_gst_category",
