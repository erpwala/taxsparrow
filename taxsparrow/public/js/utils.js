...
frappe.provide("taxsparrow");

window.gst_settings = frappe.boot.gst_settings;

Object.assign(taxsparrow, {
    get_gstin_query(party, party_type = "Company") {
        ...
        return {
            query: "taxsparrow.gst_india.utils.get_gstin_list",
            params: { party, party_type },
        };
    },

    async get_gstin_options(party, party_type = "Company") {
        const { query, params } = taxsparrow.get_gstin_query(party, party_type);
    ...
    async get_account_options(company) {
        if (!company) return;
        const { message } = await frappe.call({
            method: "taxsparrow.gst_india.utils.get_all_gst_accounts",
            args: {
                company,
            },
        });
    ...
    async set_gstin_status(field, transaction_date, force_update = 0) {
        ...
        const { message } = await frappe.call({
            method: "taxsparrow.gst_india.doctype.gstin.gstin.get_gstin_status",
            args: {
                gstin,
                transaction_date,
        ...
        field.set_description(
            taxsparrow.get_gstin_status_desc(
                message?.status,
    ...
    set_gstin_refresh_btn(field, transaction_date) {
        ...
        refresh_btn.on("click", async function () {
            ...
            await taxsparrow.set_gstin_status(
                field,
                transaction_date,
                force_update
            );
    ...
    is_api_enabled(settings) {
        ...
        return settings.enable_api && taxsparrow.can_enable_api(settings);
    },

    is_e_invoice_enabled() {
        return taxsparrow.is_api_enabled() && gst_settings.enable_e_invoice;
    },
