...
async function update_gst_details(frm, event) {
    ...
    const party_type = taxsparrow.get_party_type(frm.doc.doctype).toLowerCase();
    ...
    taxsparrow.fetch_and_update_gst_details(frm, args);
}

taxsparrow.fetch_and_update_gst_details = function (frm, args, method) {
    frappe.call({
        method: method || "taxsparrow.gst_india.overrides.transaction.get_gst_details",
        args,
        async callback(r) {
            if (!r.message) return;
...
async function _set_gstin_status(frm, gstin_field_name) {
    ...
    if (!gstin_doc) {
        gstin_doc = await taxsparrow.set_gstin_status(gstin_field, date_field.value);

        frm._gstin_doc = frm._gstin_doc || {};
        frm._gstin_doc[gstin] = gstin_doc;
    } else {
        gstin_field.set_description(
            taxsparrow.get_gstin_status_desc(gstin_doc?.status, gstin_doc?.last_updated_on)
        );
    }
