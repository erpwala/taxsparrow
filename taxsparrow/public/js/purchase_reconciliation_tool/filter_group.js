...
class _Filter extends frappe.ui.Filter {
    set_conditions_from_config() {
        ...
        this.conditions = this.conditions.filter(
            condition => taxsparrow.FILTER_OPERATORS[condition && condition[0]]
        );
    }
}

taxsparrow.FilterGroup = class FilterGroup extends frappe.ui.FilterGroup {

    constructor(opts) {
        if (!opts.parent)
            frappe.throw(__("taxsparrow.FilterGroup: Parent element not found"));
